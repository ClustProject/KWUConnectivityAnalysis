...
for i=1:sub_len  % Subject [1, 45]
    ...
    file_name = strcat('ExtractedFeaturesDistEn_8_norm_v2', '\', subject_file_struct(i+2).name);
    fprintf("Run : {%d}", i);
    trials_name_list = [];
    for j=1:trial_len % Trial [1, 15]
        ...
        for row=1:num_nodes % Nodes = 62
            ...
            extracted_waves = (extracted_waves - min(min(extracted_waves))) ./ (max(max(extracted_waves)) - min(min(extracted_waves)));
            all_waves_features = [];
            for w = 1 : num_frequency_bands % Frequency bands = 5
                ...
                for col=1:num_EEG_blocks
                    ...
                    disten = DistEn(window, 2, 8, 16);
                    disten_feature = [disten_feature, disten];
                    window_len = window_len+fs;
                end
                all_waves_features = [all_waves_features; disten_feature];
            end
        ...
        renamed = strcat('DistEn',num2str(j));
        eval([genvarname(renamed), '= trial_features;']);
        renamed = strcat("DistEn",string(j));
