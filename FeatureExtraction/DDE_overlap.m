...
move_step = fs; % 200, 400, 600, 800 : 1s, 2s, 3s, No_overlap 
n = 8;
...
for i=1:sub_len%sub_len  % Subject [1, 45]
    ...
    file_name = strcat('ExtractedFeaturesDDE_overlap', '\', subject_file_struct(i+2).name);
    ...
    for j=1:trial_len % Trial [1, 15]
        ...
        for row=1:num_nodes % Nodes = 62
            ...
            window_start = 1;
            [imf, residue, ~] = emd(EEG_data, 'MaxNumIMF', 6);
            for col=1:num_EEG_blocks
                dde_feature = [];
                wave = imf(window_start:window_start+window_size-1, :);
                for w = num_frequency_bands : -1 : 1 % Frequency bands = 5
                    %fprintf("%d, %d\n",col,window_len)
%                     if size(wave,2) <= 4
%                         fprintf('%d, %d, %d\n', row, col, j);
%                     end
                    window = wave(:,w);
                    dde = DE(window, window_size);
                    dde_feature = [dde_feature, dde];
                end
                window_start = window_start+move_step;
                all_waves_features = [all_waves_features; dde_feature];
            end
        ...
        renamed = strcat('ODDE',num2str(j));
        eval([genvarname(renamed), '= trial_features;']);
        renamed = strcat("ODDE",string(j));
