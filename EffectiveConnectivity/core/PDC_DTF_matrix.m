function [PDC, f] = PDC_DTF_matrix(A,p_opt,Fs,Fmax,Nf)
...
% 밃nalysis of the time-varying cortical neural connectivity in the newborn EEG: A time-frequency approach,�
...
PDC   = zeros(CH,CH,Nf); % Partial Directed Coherence
DTF   = zeros(CH,CH,Nf); % Directed Transfer Function

f = (0:(Nf-1))*(Fmax/Nf); % Frequency span
z = 1i*2*pi/Fs;

exponents = reshape((1i*2*pi*kron(0:(Nf-1),(1:p_opt))/Nf),p_opt,Nf).';
% A2 = [eye(CH) -squeeze(A)]; % A2(f) = [I -A1(t) -A2(t) ... -Ap(t)]
...
for n = 1 : Nf % Number of frequency points
    ...
    for k = 1 : p_opt
        A2 = eye(CH) - A(:,:,k);
%         Af = Af + A2*exp(z*(k)*f(n));
        Af = Af + A2*exp(exponents(n,k));
    end
    ...
    PDC(:,:,n)  = abs(Af)./denom_PDC(ones(1,CH),:); % Aij/(sqrt(aj'*aj))
